...
## 项目详情
### 项目简介
Bilibili（B站）视频与弹幕下载器。通过 BV 号下载视频（支持 DASH 流和传统单流格式），同时下载弹幕（protobuf 格式）并保存为 JSON Lines 文件。
...
tests/
├── test_downloader.py   # 下载器单元测试（mock HTTP 请求）
...
- `Downloader` 是最核心的类，已有完整的单元测试覆盖（含 mock HTTP 请求）
